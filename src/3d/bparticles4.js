...
var simulator = require('./bsimulator4');
...
function init(renderer) {
  ...
  _color2 = new THREE.Color('#550000');
  _meshes = [
...
function update(dt) {
  let mesh;
  const hide = !(settings.bbodyPos[3] && settings.bbodyPos[3].x && settings.bbodyPos[3].y);
  _triangleMesh.visible = !hide && settings.useTriangleParticles;
  _particleMesh.visible = !hide && !settings.useTriangleParticles;
  _tmpColor.setStyle(settings.color1);
  _color1.lerp(_tmpColor, 0.05);
  _tmpColor.setStyle('#000055');
