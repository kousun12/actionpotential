...
function update(dt) {
  ...
  if (settings.speed || settings.dieSpeed) {
    ...
    _positionShader.uniforms.speed.value = settings.bspeed * deltaRatio;
    _positionShader.uniforms.dieSpeed.value = settings.bdieSpeed * deltaRatio;
    _positionShader.uniforms.radius.value = settings.bradius;
    _positionShader.uniforms.curlSize.value = settings.curlSize;
    _positionShader.uniforms.attraction.value = settings.battraction;
    _positionShader.uniforms.initAnimation.value = exports.initAnimation;

    if (settings.followMouse) {
      _positionShader.uniforms.mouse3d.value.lerp(settings.mouse3d, 0.2);
    } else {
      _positionShader.uniforms.mouse3d.value.lerp(settings.bbodyPos[0], 0.2);
    }
